...
FROM (	
	...
	UNION (
		...
		\normalsize
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/1.jpg}
			\end{figure}

	\clearpage
	\subsection*{Consulta 2}
	\textit{La razón social de los proveedores tales que existe un producto
...
WHERE EXISTS (	
	...
	WHERE NOT EXISTS(
		...
		\normalsize
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/2.jpg}
			\end{figure}		
		
		\clearpage
		\subsection*{Consulta 3}
		\textit{Un listado alfabético de subtipos de componentes a reponer indicando tipo de componente, subtipo de componente y razon$\_$social de los proveedores que los comercializan.}
...
ORDER BY TIPO, SUBTIPO
		...
		\normalsize
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/3.jpg}
			\end{figure}	
...
WHERE NOT EXISTS (
	...
	AND NOT EXISTS(
		...
		\normalsize
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/4.jpg}
			\end{figure}		
		
		\clearpage
		\subsection*{Consulta 5}
		\textit{La razon$\_$social, el nro$\_$pedido y el subtipo de componente de los ítems de pedidos de subtipos de componente no comercializados por el proveedor del pedido.}
		
		Interpretación: Obtener los pedidos a proveedores que contienen un item pedido que ese proveedor no comercializa.
...
WHERE 	PR.CUIT = PE.CUIT 
		...
		\normalsize
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/5.jpg}
			\end{figure}		
		...
		\subsubsection*{Interpretacion 1}
			La razón social de los proveedores que proveen un solo tipo de componente.
...
AND NOT EXISTS (
	...
	AND S1.TIPO <> S2.TIPO)
		...
		\normalsize
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/6a.jpg}
			\end{figure}	
		
		\subsubsection*{Interpretacion 2}
		Interpretación: La razón social de los proveedores que tienen en su catalogo un tipo de componente que no esta en el catalogo de ningún otro proveedor.
...
AND NOT EXISTS(
	...
	AND S.TIPO = S2.TIPO)	
		...
		\normalsize
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/6b.jpg}
			\end{figure}		
		...
		Hipótesis: Lo entregado se encuentra en ITEM$\_$PEDIDO y tiene cantidad recibida mayor a cero.
...
AND NOT EXISTS (
	...
	AND RAZON_SOCIAL<>'Ceven S.A.')
		...
		\normalsize
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/7.jpg}
			\end{figure}		
...
WHERE NOT SUB.CANT_STOCK=(
	...
	and  COMP.NRO_ORDEN IS NULL)
		...
		\normalsize
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/8.jpg}
			\end{figure}		
		
		\clearpage
		\subsection*{Consulta 9}
		\textit{El nro$\_$pedido y el subtipo de componentes de los ítems de pedidos cuya cantidad pedida supere a la cantidad pedida promedio de ese subtipo de componentes.}
...
WHERE ITEM.CANT_PEDIDA>( 
	...
	WHERE ITEM2.SUBTIPO=ITEM.SUBTIPO)			   
		...
		\normalsize
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/9.jpg}
			\end{figure}		
		...
		\normalsize		
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/10.jpg}
			\end{figure}		
...
WHERE NOT EXISTS(
	...
	AND PR.CUIT NOT IN (
		...
		\normalsize
			...
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{resultados/11.jpg}
			\end{figure}		
