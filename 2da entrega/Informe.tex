...
WHERE 	PR.CUIT = PE.CUIT 
		...
		En este caso el enunciado resulta ambigüo y surgen dos interpretaciones de la definición de la consulta, por lo cual proveemos solución para ambas.
		
		\subsubsection*{Interpretación 1}
...
AND NOT EXISTS (
	...
	AND S1.TIPO <> S2.TIPO)
		...
		\subsubsection*{Interpretación 2}
		La razón social de los proveedores que tienen en su catalogo un tipo de componente que no esta en el catalogo de ningún otro proveedor.
...
AND NOT EXISTS (
	...
	AND S.TIPO = S2.TIPO )
		...
		Hipótesis: Los items entregados son los que se encuentran en ITEM$\_$PEDIDO con CANT$\_$RECIBIDA mayor a cero.
