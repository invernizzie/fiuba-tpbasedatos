...
que no comercializan.\\
		...
		Hipótesis: Si lo que pregunta es sobre si los subtipos que puede pedir entonces:
		...
		WHERE SUB.SUBTIPO IN (
			...
			WHERE SUB.SUBTIPO=CAT.SUBTIPO
			AND CAT.CUIT=PROV.CUIT
			AND PROV.RAZON_SOCIAL='Ceven S.A.')
		AND SUB.SUBTIPO NOT IN(
			SELECT SUB.SUBTIPO
			FROM SUBTIPO_COMPONENTE SUB, CATALOGO_PROVEEDOR CAT, PROVEEDOR PROV
			WHERE SUB.SUBTIPO=CAT.SUBTIPO 
			AND CAT.CUIT=PROV.CUIT 
		...
		Hipótesis: Si lo que pregunta es sobre lo que efectivamente se pidio, entonces:		
